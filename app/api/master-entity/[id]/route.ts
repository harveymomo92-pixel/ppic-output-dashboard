...
export async function PUT(request: NextRequest, context: RouteContext) {
  ...
  try {
    const result = db.prepare(`
      UPDATE master_entity_target SET
        area_kerja_line = ?,
        kode_asli_sistem = ?,
        kode_asli_normalized = ?,
        display_laporan = ?,
        deskripsi_produk = ?,
    ...
    const row = db.prepare('SELECT * FROM master_entity_target WHERE id = ?').get(Number(id)) as DbRow;
    return NextResponse.json({ data: rowToClient(row) });
  } finally {
    db.close();
  }
}
...
export async function DELETE(_request: NextRequest, context: RouteContext) {
  ...
  const db = getDb();
  try {
    const result = db.prepare('DELETE FROM master_entity_target WHERE id = ?').run(Number(id));
    if (result.changes === 0) return NextResponse.json({ error: 'Not found' }, { status: 404 });
